...
def find_new_voxels(sx, sy, sz, iteration):
  ...
  # Fixed x axis, iterate from y axis and z axis
  for j in range(sy - iteration, sy + iteration+1):
      for k in range(sz - iteration, sz + iteration+1):
          new_voxel_coordinates.append([sx - iteration, j, k])
          new_voxel_coordinates.append([sx + iteration, j, k])

  # Fixed y axis, iterate from x axis and z axis
  for i in range(sx - iteration + 1, sx + iteration):
      for k in range(sz - iteration, sz + iteration+1):
          new_voxel_coordinates.append([i, sy - iteration, k])
          new_voxel_coordinates.append([i, sy + iteration, k])

  # Fixed y axis, iterate from x axis and z axis
  for i in range(sx - iteration + 1, sx + iteration):
      for j in range(sy - iteration + 1, sy + iteration):
          new_voxel_coordinates.append([i, j, sz - iteration])
          new_voxel_coordinates.append([i, j, sz + iteration])
...
def grow_from_seed(inputVolumeData, size, seed_location, global_intensity_diff, local_intensity_diff, outputROIData):
  ...
  while iteration < maxIteration:
      ...
      print('[Iter {}] the shape of new_voxel_coordinates in this iteration: {}'.format(iteration, voxels.shape))
      l, w = voxels.shape
      for co in range(l):
          vx, vy, vz = voxels[co, ]
          candidate = inputVolumeData[vx, vy, vz]

          neighboursIndex = find_new_voxels(vx, vy, vz, 1)

          # Convert to array value to save time
          neighboursIndex = np.asarray(neighboursIndex)
          #print(neighboursIndex.shape)

          # Caculate the corresponding local intensity range
          n_l, n_w = neighboursIndex.shape
          minLocal = 1000
          maxLocal = -999
          for index in range(n_l):
              nx, ny, nz = neighboursIndex[index, ]
              neighbour = inputVolumeData[nx, ny, nz]
              #minLocal = 1000
              #maxLocal = -999
              #print(neighbour)
              if outputROIData[nx, ny, nz] == 1:
                  # print(neighbour - local_intensity_diff)
          ...
          #print(ROI_min, ROI_max, minLocal, maxLocal)
          #print(inputVolumeData[vx, vy, vz])
          #print(outputROIData[vx, vy, vz])
          if candidate >= ROI_min and candidate <= ROI_max:
              #print("global")
              #print(outputROIData[vx, vy, vz])
              if candidate >= minLocal and candidate <= maxLocal:
