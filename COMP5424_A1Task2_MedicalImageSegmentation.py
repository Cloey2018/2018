...
def find_new_voxels(sx, sy, sz, iteration):
  ...
  #Add your code below    #
  #########################
  previous_voxel_coordinates = []

  # Add elements to new voxel list
  for i in range(sx - iteration, sx + iteration+1):
      for j in range(sy - iteration, sy + iteration+1):
          for k in range(sz - iteration, sz + iteration+1):
              new_voxel_coordinates.append([i, j, k])

  # Add elements to previous voxel list
  for a in range(sx - (iteration-1), sx + iteration):
      for b in range(sy - (iteration-1), sy + iteration):
          for c in range(sz - (iteration-1), sz + iteration):
              previous_voxel_coordinates.append([a, b, c])

  # Remove previous elements from the new_voxel_coordinates
  for voxel in previous_voxel_coordinates:
      new_voxel_coordinates.remove(voxel)
...
def grow_from_seed(inputVolumeData, size, seed_location, global_intensity_diff, local_intensity_diff, outputROIData):
  ...
  outputROIData = np.zeros((dx, dy, dz))
  outputROIData[sx, sy, sz] = 1
  maxIteration = min(sx, (dx - sx - 1), sy, (dy - sy - 1), sz, (dz - sz - 1))

  # Stopping criterion: reach the boundary of the image
  while iteration <= maxIteration:
      voxels = find_new_voxels(sx, sy, sz, iteration)
      for voxel in voxels:
          vx, vy, vz = voxel
          candidate = inputVolumeData[vx, vy, vz]
          neighboursIndex = []
          # Find neighbours of candidate
          # check candidates without 26 neighbours
          if iteration == maxIteration:
              for i in range(vx - 1, vx + 2):
                  for j in range(vy - 1, vy + 2):
                      for k in range(vz - 1, vz + 2):
                          if (i in range(dx) and j in range(dy) and k in range(dz)):
                              neighboursIndex.append([i, j, k])
          else: # candidates with full 26 neighbours
              for i in range(vx - 1, vx + 2):
                  for j in range(vy - 1, vy + 2):
                      for k in range(vz - 1, vz + 2):
                          neighboursIndex.append([i, j, k])

          # Delete the index of candidate from the neighboursIndex list
          neighboursIndex.remove(voxel)

          # Caculate the corresponding local intensity range
          for index in neighboursIndex:
              neighbour = inputVolumeData[index[0], index[1], index[2]]
              minLocal = 0
              maxLocal = 0
              if outputROIData[index[0], index[1], index[2]] == 1:
                  minLocal = min(minLocal, neighbour - local_intensity_diff)
                  maxLocal = max(maxLocal, neighbour + local_intensity_diff)

          # GlobalChecking && LocalChecking
          # new range combined global and local
          new_min = max(ROI_min, minLocal)
          new_max = min(ROI_max, maxLocal)
          if candidate >= new_min and candidate <= new_max:
              outputROIData[vx, vy, vz] = 1
